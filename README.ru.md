...
Итого в пакете `ZMDMDOC`: **5 программ, 2 интерфейса, 21 класс** (13 ядро + 2
генерируемых + 6 MDG-сценарий) + message class `ZMDMDOC`.
...
3. Прогнать юнит-тесты пакета: `Ctrl+Shift+F10` в ADT — ожидается **238 зелёных**
...
- Сжатые PDF (zlib /FlateDecode, RFC 1950) распаковывает pure-ABAP `ZCL_MDMDOC_INFLATE` —
  ядерные классы не могут завершить голый zlib-поток, потому что проверяют контрольные суммы,
  которых в нём нет. `ZMDMDOC_DOCTOR` печатает чеки inflate и байт-прозрачности кодовой
  страницы. Первое, что стоит проверить на целевой системе: эти два чека, затем `/UI2/CL_JSON`.
