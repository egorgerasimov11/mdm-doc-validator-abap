...
1. **Инфляция zlib-потоков (RFC 1950) — `ZCL_MDMDOC_INFLATE`.**
   PDF использует FlateDecode = zlib, а не gzip. Ядерные классы здесь не годятся: они проверяют
   контрольные суммы (gzip CRC32, ZIP CRC), которых у голого zlib-потока нет, — этот риск
   реализовался как кейс C-2026-08-20-01. Распаковку делает pure-ABAP `ZCL_MDMDOC_INFLATE`
   (без CRC-проверок); `CL_ABAP_GZIP` остаётся первой дешёвой попыткой на случай настоящего
   gzip. Проверка: чеки `inflate` и `codepage` в `ZMDMDOC_DOCTOR`, затем реальный сжатый PDF
   в dev.
